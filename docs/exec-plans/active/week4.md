...
- [ ] `DataInitializer` — `@Component` + `ApplicationRunner` + `@Profile("local")`
...
- [ ] `/reviews` 페이지 브라우저에서 정상 렌더링
- [ ] `/reviews/{id}` 비로그인 시 답변 블러 UI 확인
- [ ] `/reviews/{id}` source 보기 → `<title>`, `<meta>` SEO 태그 확인
- [ ] 회원가입/로그인 폼 동작
- [ ] DataInitializer: `@Profile("local")`에서 회사 50개 + 후기 20건 정상 삽입
- [ ] `./gradlew test` 전체 통과
...
- [ ] `docker-compose.yml` — app + mysql + redis 로컬 실행용
